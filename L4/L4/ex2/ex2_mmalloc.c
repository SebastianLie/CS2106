...
void* malloc(int size)
{
	...
	partMetaInfo *bestPartition = NULL;
	...
	int bestSize = -1;
	while (current!=NULL){
		if(current->status == FREE && current->size >= size && current->size > bestSize) {
	...
	if (bestPartition == NULL){	//heap full
		return NULL;
	}	
	...
	// meta info structure 
	if (bestPartition->size >= size + hmi.partMetaSize + 4 ){
		splitPart(bestPartition, size);
	}

	bestPartition->status = OCCUPIED;

	return (void*)bestPartition + hmi.partMetaSize;
}
