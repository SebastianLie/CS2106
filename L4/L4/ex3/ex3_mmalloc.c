...
void free(void* address)
{
	...
	partMetaInfo *current = hmi.base; 
	while(current != NULL && current->status == OCCUPIED) { 
		current = current->nextPart;		
	} while(current != NULL) {
		 while(current->nextPart != NULL && (current->nextPart)->status == FREE) { 
...
void compact()
{
    ...
    // too. Look into memmove() library call 
     partMetaInfo *current = hmi.base;
     partMetaInfo *lastOccupied = NULL;
     void* startAddress = (void*) hmi.base;
     void* occupiedAddress = (void*) hmi.base;
     hmi.base = NULL;
	

    int occupiedSize = 0;
    int unOccupiedSize = 0;

     while(current != NULL) {	
     	if(current->status == FREE) {
		unOccupiedSize += (current->size + hmi.partMetaSize);
	} else {
		memmove(occupiedAddress, startAddress, current->size + hmi.partMetaSize);
		current = occupiedAddress;
		if(hmi.base == NULL ) {
			hmi.base = current;
		}
		
		if(lastOccupied != NULL) {
			lastOccupied->nextPart = current;
		}

		occupiedAddress += (current->size + hmi.partMetaSize);
		occupiedSize += (current->size + hmi.partMetaSize);
		lastOccupied = current;
	}
	startAddress += (current->size + hmi.partMetaSize);
	current = current->nextPart;
     }

     void* freeAddress = (void*) hmi.base + occupiedSize;
     partMetaInfo *freePartition = freeAddress;
     freePartition->status = FREE;
     freePartition->size = unOccupiedSize - hmi.partMetaSize;
     freePartition->nextPart = NULL; 
      
     if(lastOccupied != NULL) {
	lastOccupied->nextPart = freePartition;
     } else {
